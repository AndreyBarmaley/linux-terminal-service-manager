...
namespace LTSM {
    ...
    AVPixelFormat Tools::AV_PixelFormatEnumFromMasks(int bpp, uint32_t rmask, uint32_t gmask, uint32_t bmask, uint32_t amask, bool debug) {
        if(debug) {
            ...
            bool bigEndian = true;
#endif
            Application::info("{}: pixel format, bpp: %d, rmask: {:#08x}, gmask: {:#08x}, bmask: {:#08x}, amask: {:#08x}, be: %d",
        ...
        Application::error("{}: unsupported pixel format, bpp: %d, rmask: {:#08x}, gmask: {:#08x}, bmask: {:#08x}, amask: {:#08x}",
