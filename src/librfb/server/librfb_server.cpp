...
namespace LTSM {
    ...
    void ClientEncodings::setPriority(const std::vector<int> & priorities) {
        encs.remove_if([&](auto & enc) {
            return std::any_of(priorities.begin(), priorities.end(), [&](auto & val) { return val == enc; });
        });
    ...
    int ClientEncodings::findPriorityFrom(std::initializer_list<int> priorities) const {
        for(const auto & enc : encs) {
            if(std::any_of(priorities.begin(), priorities.end(),[&](auto & val) { return val == enc; })) {
                return enc;
            }
        }
    ...
    void RFB::ServerEncoder::recvSetEncodings(void) {
        ...
        while(0 < numEncodings--) {
            ...
            if(! disabledEncodings.empty()) {
                ...
                if(std::any_of(disabledEncodings.begin(), disabledEncodings.end(), [&](auto & str) { return enclower == Tools::lower(str); })) {
    ...
    void RFB::ServerEncoder::serverSelectClientEncoding(void) {
        ...
        switch(compatible) {
            ...
            case RFB::ENCODING_ZLIB: {
                ...
                if(auto it = std::find_if(clevels.begin(), clevels.end(), [this](auto & enc) {
    ...
    void RFB::ServerEncoder::sendEncodingLtsmCursor(const FrameBuffer & fb, uint16_t xhot, uint16_t yhot) {
        ...
        // cursor rgba data
        if(std::none_of(cursorSended.begin(), cursorSended.end(), [&cursorId](auto & curid) { return curid == cursorId; })) {
