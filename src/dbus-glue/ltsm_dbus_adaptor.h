...
protected:
        ...
        : object_(object)
    {
        ...
        object_.registerMethod("busSetAuthenticateInfo").onInterface(INTERFACE_NAME).withInputParamNames("display", "login", "password").withOutputParamNames("result").implementedAs([this](const int32_t& display, const std::string& login, const std::string& password){ return this->busSetAuthenticateInfo(display, login, password); });
...
private:
    ...
    virtual bool busSetAuthenticateInfo(const int32_t& display, const std::string& login, const std::string& password) = 0;
