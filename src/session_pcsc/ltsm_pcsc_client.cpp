...
std::unique_ptr<LTSM::Channel::ConnectorBase>
    LTSM::Channel::createClientPcscConnector(uint8_t channel, const std::string & url, const ConnectorMode & mode, const Opts & chOpts, ChannelClient & sender)
{
...
/// ConnectorClientPcsc
LTSM::Channel::ConnectorClientPcsc::ConnectorClientPcsc(uint8_t ch, const std::string & url, const ConnectorMode & mod, const Opts & chOpts, ChannelClient & srv)
...
void LTSM::Channel::ConnectorClientPcsc::pushData(std::vector<uint8_t> && recv)
{
    ...
    if(4 < sb.last())
    {
        ...
        if(pcscInit == PcscOp::Init)
        {
            ...
            switch(pcscCmd)
            {
                case PcscLite::EstablishContext:  return pcscEstablishContext(sb);
                case PcscLite::ReleaseContext:    return pcscReleaseContext(sb);
                case PcscLite::ListReaders:       return pcscListReaders(sb);
                case PcscLite::Connect:           return pcscConnect(sb);
                case PcscLite::Reconnect:         return pcscReconnect(sb);
                case PcscLite::Disconnect:        return pcscDisconnect(sb);
                case PcscLite::BeginTransaction:  return pcscBeginTransaction(sb);
                case PcscLite::EndTransaction:    return pcscEndTransaction(sb);
                case PcscLite::Transmit:          return pcscTransmit(sb);
                case PcscLite::Status:            return pcscStatus(sb);
                case PcscLite::GetStatusChange:   return pcscGetStatusChange(sb);
                case PcscLite::Control:           return pcscControl(sb);
                case PcscLite::Cancel:            return pcscCancel(sb);
                case PcscLite::GetAttrib:         return pcscGetAttrib(sb);
                case PcscLite::SetAttrib:         return pcscSetAttrib(sb);
...
void LTSM::Channel::ConnectorClientPcsc::pcscConnect(const StreamBufRef & sb)
{
    ...
    SCARDHANDLE hCard = 0; DWORD activeProtocol = 0;
...
void LTSM::Channel::ConnectorClientPcsc::pcscReconnect(const StreamBufRef & sb)
{
    ...
    Application::info("%s: handle: %" PRIx64 ", shareMode: %" PRIu32 ", prefferedProtocols: %" PRIu32 ", initialization: %" PRIu32,
...
void LTSM::Channel::ConnectorClientPcsc::pcscStatus(const StreamBufRef & sb)
{
    ...
    DWORD state = 0; DWORD protocol = 0;
    char readerName[MAX_READERNAME]; DWORD readerNameLen = sizeof(readerName);
    BYTE atrBuf[MAX_ATR_SIZE]; DWORD atrLen = sizeof(atrBuf);
...
void LTSM::Channel::ConnectorClientPcsc::pcscGetAttrib(const StreamBufRef & sb)
{
    ...
    std::vector<BYTE> attrBuf(MAX_BUFFER_SIZE); DWORD attrLen = MAX_BUFFER_SIZE;
...
void LTSM::Channel::ConnectorClientPcsc::pcscSetAttrib(const StreamBufRef & sb)
{
    ...
    Application::info("%s: handle: %" PRIx64 ", attrId: %" PRIu32 ", attrLen: %" PRIu32, __FUNCTION__, hCard, attrId, attrLen);
