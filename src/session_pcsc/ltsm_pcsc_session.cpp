...
namespace LTSM
{
    ...
    PcscClient::PcscClient(int fd, PcscSessionBus* sessionBus)
    {
        ...
        thread = std::thread([this, sessionBus]()
        {
            ...
            while(! this->shutdown)
            {
                ...
                {
                    Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32, "ClientContextThread", this->sock.fd(), this->context);
        ...
        while(! pcscShutdown)
        {
            ...
            if(NetworkStream::hasInput(socketFd, 1))
            {
                if(auto sock = UnixSocket::accept(socketFd); 0 < sock)
                {
                    ...
                    clients.remove_if([](auto & st){ return ! st.shutdown; });
    ...
    bool PcscSessionBus::pcscEstablishContext(PcscClient & st, uint32_t len)
    {
        ...
        if(ret == SCARD_S_SUCCESS)
        {
            ...
            Application::debug("%s: client id: %" PRId32 ", remote context: 0x%016" PRIx64 ", local context: 0x%08" PRIx32, __FUNCTION__, st.id(), remoteContext, context);
            return true;
        }
    ...
    PcscLite::ReaderState* PcscSessionBus::findReaderState(const std::string & name)
    {
        ...
        auto it = std::find_if(readers.begin(), readers.end(),
                    [&](auto & rd) { return 0 == name.compare(rd.name); });
    ...
    bool PcscSessionBus::pcscConnect(PcscClient & st, uint32_t len)
    {
        ...
        Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", shareMode: %" PRIu32 ", prefferedProtocols: %" PRIu32 ", reader: `%s'",
        ...
        if(ret == SCARD_S_SUCCESS)
        {
            ...
            Application::debug("%s: client id: %" PRId32 ", remote handle: 0x%016" PRIx64 ", local handle: 0x%08" PRIx32 ", activeProtocol: %" PRIu32, __FUNCTION__, st.id(), remoteHandle, handle, activeProtocol);
            return true;
        }
    ...
    bool PcscSessionBus::pcscReconnect(PcscClient & st, uint32_t len)
    {
        ...
        Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", handle: 0x%08" PRIx32 ", shareMode: %" PRIu32 ", prefferedProtocols: %" PRIu32 ", inititalization: %" PRIu32,
    ...
    bool PcscSessionBus::pcscDisconnect(PcscClient & st, uint32_t len)
    {
        ...
        Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", handle: 0x%08" PRIx32 ", disposition: %" PRIu32,
    ...
    bool PcscSessionBus::pcscTransmit(PcscClient & st, uint32_t len)
    {
        ...
        {
            Application::error("%s: client id: %" PRId32 ", invalid length, send: %" PRIu32 ", data: %u", __FUNCTION__, st.id(), sendLength, data.size());
            return false;
        }
        ...
        Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", handle: 0x%08" PRIx32 ", pciProtocol: 0x%08" PRIx32 ", pciLength: %" PRIu32 ", send size: %" PRIu32,
    ...
    bool PcscSessionBus::pcscGetAttrib(PcscClient & st, uint32_t len)
    {
        ...
        Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", attrId: %" PRIu32, __FUNCTION__, st.id(), st.context, attrId);
    ...
    bool PcscSessionBus::pcscSetAttrib(PcscClient & st, uint32_t len)
    {
        ...
        Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", attrId: %" PRIu32 ", attrLength %" PRIu32, __FUNCTION__, st.id(), st.context, attrId, attrLen);
    ...
    bool PcscSessionBus::pcscGetVersion(PcscClient & st, uint32_t len)
    {
        ...
        Application::debug("%s: client id: %" PRId32 ", protocol version: %" PRIu32 ".%" PRIu32, __FUNCTION__, st.id(), versionMajor, versionMinor);
    ...
    bool PcscSessionBus::pcscGetReaderState(PcscClient & st, uint32_t len)
    {
        ...
        Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", readers length: %" PRIu32, __FUNCTION__, st.id(), st.context, readersLength);
    ...
    {
        Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", timeout: %" PRIu32, __FUNCTION__, st->id(), st->context, timeout);
    ...
    bool PcscSessionBus::pcscReaderStateChangeStart(PcscClient & st, uint32_t len)
    {
        ...
        if(st.versionMajor == 4 && st.versionMinor > 2)
        {
            // new protocol 4.4: empty params
            Application::debug( "%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", timeout: %" PRIu32, __FUNCTION__, st.id(), st.context);
    ...
    bool PcscSessionBus::pcscCancel(PcscClient & st, uint32_t len)
    {
        ...
        const std::scoped_lock guard{ clientsLock };
        auto it = std::find_if(clients.begin(), clients.end(), [&](auto & cl) { return cl.context == context; });
    ...
    std::list<std::string> PcscSessionBus::pcscListReaders(PcscClient & st)
    {
        ...
        Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", readers count: %" PRIu32, __FUNCTION__, st.id(), st.context, readersCount);
    ...
    int64_t PcscSessionBus::syncReaders(PcscClient & st, bool* changed)
    {
        ...
        for(auto & rd : readers)
        {
            ...
            auto it = std::find_if(names.begin(), names.end(),
                        [&](auto & name){ return 0 == name.compare(rd.name); });
    ...
    int64_t PcscSessionBus::pcscGetStatusChange(PcscClient & st, uint32_t timeout, SCARD_READERSTATE* states, uint32_t statesCount)
    {
        if(! ltsm)
        {
            Application::error("%s: no service", __FUNCTION__);
        ...
        Application::debug("%s: client id: %" PRId32 ", context: 0x%08" PRIx32 ", timeout: %" PRIu32 ", states: %" PRIu32, __FUNCTION__, st.id(), st.context, timeout, counts);
    ...
    int PcscSessionBus::syncReaderStatusChange(PcscClient & st, const std::string & readerName, PcscLite::ReaderState & rd, bool* changed)
    {
        ...
        if(state.dwEventState & SCARD_STATE_CHANGED)
        {
            ...
            rd.state = state.dwEventState & SCARD_STATE_PRESENT ? (PcscLite::StatePresent | PcscLite::StatePowered | PcscLite::StateNegotiable) : PcscLite::StateAbsent;
